...
const gateMatches = [
  ["MVP legacy", stats.mvp.assertionsPass, stats.mvp.assertionsFail, stats.mvp.assertionsSkip, "assertions"],
  ["SIMD", stats.suiteAssertions.simd, 0, 0, "assertions"],
];
const wazero = match(
...
const evidenceMap = {
  ...
  verification: evidence(commit, "SPECTEST.md", "published conformance baselines", "#L1-L20"),
...
const facts = {
  ...
  compatibility: {
    verification: {
      wagoCommit: commit,
      host: "published repository baselines",
      pass: stats.verification.checksPass,
      fail: stats.verification.checksFail,
      skip: stats.verification.checksSkip,

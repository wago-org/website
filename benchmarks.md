...
Host-boundary Wago source commit: 0207bd7ec7900f703c246c71c9e6644892fdf87f
...
- The Host → Wasm row uses Wago `Instance.Invoke` and wazero's resolved public function call on the same `(i32) → i32` identity export. Seven samples per architecture were measured with `GOMAXPROCS=1`, `WAGO_BOUNDS=signals`, `-tags wago_guardpage`, and `-benchtime=700ms`; the displayed values are medians. Wago's call returned an instance-owned result slice with zero Go heap allocations.
...
```sh
GOWORK=off GOMAXPROCS=1 WAGO_BOUNDS=signals go test -tags wago_guardpage ./suite -run '^$' -bench '^BenchmarkExec(InstanceCall_wago|CallOverhead_wazero)$' -benchmem -benchtime=700ms -count=7
```
